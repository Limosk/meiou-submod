...
# also save all river_connections as province_flags ("river_province" and "river_to_####")"""
    ...
    for x in range(27, 1245):
        rgb2temp4[img.getpixel((x, y))] = (x - 27) / 1218
    ...
    for i,prov in enumerate(provinces): #or just land_provinces?
        ...
        s += "\n\t\t%4d = {%s%s%s%s%s%s%s%s%s%s%s%s%s  }" % (prov, prov_id, coords, port, portdist, seazone, sizes, rivers, soil, heatAvg, heatSeasonal, heatDaily, rain, inun)
